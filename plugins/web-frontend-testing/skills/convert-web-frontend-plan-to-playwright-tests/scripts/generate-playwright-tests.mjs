...
function compileStep(step) {
    switch (step.action) {
        ...
        case "screenshot":
            return `await page.screenshot({ path: ${stringLiteral(step.value ?? `web-frontend-${Date.now()}.png`)}, fullPage: true });`;
        default:
...
test.describe("web frontend regression", () => {
...
export function generatePlaywrightTestsFromPlan(plan, options = {}) {
    const outDir = path.resolve(options.outDir ?? "tests/web-frontend");
    const validation = validatePlan(plan);

    if (validation.errors.length > 0) {
        return { generated: [], skipped: [], errors: validation.errors, warnings: validation.warnings };
    }
    ...
    for (const { scenario } of collectScenarios(plan)) {
        if (scenario.convert_to_regression_test !== true) {
            skipped.push({ scenarioId: scenario.id, reason: "convert_to_regression_test is not true" });
            continue;
        }
        if (!Array.isArray(scenario.executable_steps) || scenario.executable_steps.length === 0) {
            skipped.push({ scenarioId: scenario.id, reason: "no executable_steps defined" });
            continue;
        }
        ...
        let code;
        try {
            code = compileScenario(scenario);
        } catch (err) {
            return {
                generated,
                skipped,
                errors: [`scenario "${scenario.id}": ${err.message}`],
    ...
    return { generated, skipped, errors: [], warnings: validation.warnings };
}
...
function runCli() {
    ...
    const outDir = arg("out", "tests/web-frontend");

    if (!planPath) {
        console.error(
            "Usage: node skills/convert-web-frontend-plan-to-playwright-tests/scripts/generate-playwright-tests.mjs --plan <path-to-plan.yaml> [--out tests/web-frontend]"
        );
        process.exit(2);
    }
    ...
    for (const skipped of result.skipped) {
        console.log(`Skipped ${skipped.scenarioId}: ${skipped.reason}`);
    }

    if (result.generated.length === 0) {
        console.log("No regression-eligible scenarios found.");
    }
}
