...
const targetUrl = process.env.WEB_UX_GREMLIN_TARGET_URL || 'http://localhost:3000/admin';
...
test.describe('Admin page creation UX resilience', () => {
  test('baseline happy path', { annotation: [{ type: 'web-ux-gremlin-baseline', description: 'true' }] }, async ({ page }) => {
    ...
    await test.step('Fill required fields and confirm', async () => {
      await page.getByLabel(/title/i).fill('web-ux-gremlin-page-example');
      await page.getByRole('button', { name: /next/i }).click();
      await page.getByRole('button', { name: /confirm|create/i }).click();
    });
    await expect(page.getByText(/web-ux-gremlin-page-example/i)).toBeVisible();
  });

  test(
    'double-submit-confirm: double-submit final confirmation',
    { annotation: [{ type: 'web-ux-gremlin-scenario', description: 'double-submit-confirm' }, { type: 'web-ux-gremlin-risk', description: 'high' }] },
    async ({ page }) => {
      await page.goto(targetUrl);
      ...
      await expect(page.getByRole('row', { name: /web-ux-gremlin-page-example/i })).toHaveCount(1);
    }
  );

  test(
    'keyboard-only-create: keyboard-only page creation',
    { annotation: [{ type: 'web-ux-gremlin-scenario', description: 'keyboard-only-create' }, { type: 'web-ux-gremlin-risk', description: 'high' }] },
    async ({ page }) => {
      await page.goto(targetUrl);
