...
4. Confirm Playwright agent/MCP availability when the selected tool is MCP, including `planner_setup_page` and `planner_save_plan` for plan-stage work, plus `generator_setup_page`, `generator_read_log`, and `generator_write_test` for generate-stage work.
